...
function createPlaceholder(instruction) {
  var placeholderName = getPlaceholderName(instruction);
...
var tornadoLeafSymbols = (function () {
  return {
    insert_TORNADO_PARTIAL: function insert_TORNADO_PARTIAL(instruction, code) {
      ...
      if (instruction.state !== STATES.HTML_ATTRIBUTE) {
        var fragment = "      " + createPlaceholder(instruction) + ";\n";
        var renderer = "      td." + util.getTdMethodName("replaceNode") + "(root." + getPlaceholderName(instruction) + ", td." + util.getTdMethodName("getPartial") + "('" + key + "', " + context + ", this));\n";
        code.push(tdBody, { fragment: fragment, renderer: renderer });
      } else {
    ...
    insert_TORNADO_REFERENCE: function insert_TORNADO_REFERENCE(instruction, code) {
      ...
      if (state !== STATES.HTML_ATTRIBUTE) {
        var fragment = "      " + createPlaceholder(instruction) + ";\n";
        var renderer = "      td." + util.getTdMethodName("replaceNode") + "(root." + getPlaceholderName(instruction) + ", td." + util.getTdMethodName("createTextNode") + "(td." + util.getTdMethodName("get") + "(c, " + JSON.stringify(key) + ")));\n";
        code.push(tdBody, { fragment: fragment, renderer: renderer });
      } else {
...
var tornadoBodySymbols = (function () {
  ...
  return {
    ...
    tdBody_exists: function tdBody_exists(instruction, code) {
      ...
      if (state !== STATES.HTML_ATTRIBUTE) {
        var fragment = "      " + createPlaceholder(instruction) + ";\n";
        var renderer = "      td." + util.getTdMethodName(bodyType) + "(td." + util.getTdMethodName("get") + "(c, " + JSON.stringify(key) + "), root." + getPlaceholderName(instruction) + ", " + bodiesHash + ", c);\n";
    ...
    tdBody_section: function tdBody_section(instruction, code) {
      ...
      var placeholderNode = isInHtmlAttribute ? "null" : "root." + getPlaceholderName(instruction);
    ...
    tdBody_block: function tdBody_block(instruction, code) {
      ...
      if (state !== STATES.HTML_ATTRIBUTE) {
        var fragment = "      " + createPlaceholder(instruction) + ";\n";
        var renderer = "      td." + util.getTdMethodName("replaceNode") + "(root." + getPlaceholderName(instruction) + ", td." + util.getTdMethodName("block") + "('" + blockName + "', " + blockIndex + ", c, this));\n";
        code.push(parentTdBody, { fragment: fragment, renderer: renderer });
      } else {
    ...
    tdBody_helper: function tdBody_helper(instruction, code) {
      ...
      if (state !== STATES.HTML_ATTRIBUTE) {
        var fragment = "      " + createPlaceholder(instruction) + ";\n";
        var renderer = "      td." + util.getTdMethodName("helper") + "('" + key.join(".") + "', root." + getPlaceholderName(instruction) + ", c, " + paramsHash + ", " + bodiesHash + ");\n";
        code.push(parentTdBody, { fragment: fragment, renderer: renderer });
      }
    }
  };
