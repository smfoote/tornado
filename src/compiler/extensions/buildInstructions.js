...
let instructionDefs = {
  ...
  TORNADO_PARTIAL(node, ctx, frameStack, stateStack) {
    ...
    ctx.pushInstruction(new Instruction('insert', {key: node[1].key, item: node.stackItem, frameStack: [child, parent], ctx, stateStack: stateStack.toArray()}));
  },
  TORNADO_BODY: {
    enter(node, ctx, frameStack, stateStack) {
      ...
      let out = {
        type: 'open',
        options: {key: node[1].key, item: node.stackItem, frameStack: [child, parent], ctx, stateStack: stateStack.toArray()}
      };
    ...
    leave(node, ctx, frameStack, stateStack) {
      ...
      let out = {
        type: 'close',
        options: {item: node.stackItem, frameStack: [child, parent], ctx, stateStack: stateStack.toArray()}
      };
      return out;
    }
  },
  TORNADO_REFERENCE(node, ctx, frameStack, stateStack) {
    let child, parent;
    frameStack.pushPh();
    child = frameStack.current();
    frameStack.popPh();
  ...
  TORNADO_COMMENT(node, ctx, frameStack, stateStack) {
    let child, parent;
    frameStack.pushPh();
    child = frameStack.current();
    frameStack.popPh();
    parent = frameStack.current();
    ctx.pushInstruction(new Instruction('insert', {item: node.stackItem, frameStack: [child, parent], ctx, stateStack: stateStack.toArray()}));
  },
  HTML_ELEMENT: {
    enter(node, ctx, frameStack, stateStack) {
      let parent = frameStack.current();
      ...
      let out = {
        type: 'open',
        options: {key: node[1].tag_info.key, item: node.stackItem, frameStack: [child, parent], ctx, stateStack: stateStack.toArray()}
      };
      enterAll(this.getStates(node), stateStack);
      return out;
    },
    leave(node, ctx, frameStack, stateStack){
      ...
      let out = {
        type: 'close',
        options: {item, frameStack: [child, parent], ctx, stateStack: stateStack.toArray()}
      };
      return out;
    }
  },
  ...
  HTML_COMMENT(node, ctx, frameStack, stateStack) {
    let child = frameStack.current(),
        parent = child;
    ctx.pushInstruction(new Instruction('insert', {item: node.stackItem, frameStack: [child, parent], ctx, stateStack: stateStack.toArray()}));
  },
  PLAIN_TEXT(node, ctx, frameStack, stateStack) {
    let child = frameStack.current(),
        parent = child;
    ctx.pushInstruction(new Instruction('insert', {item: node.stackItem, frameStack: [child, parent], ctx, stateStack: stateStack.toArray()}));
  }
